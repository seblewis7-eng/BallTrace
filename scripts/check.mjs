...
if (mode === "--typecheck") {
  const [html, app, worker, core, trajectory, guard, frameFallback, adaptive, strict] = await Promise.all([readFile(path.join(root, "index.html"), "utf8"), readFile(path.join(root, "app.js"), "utf8"), readFile(path.join(root, "tracker.worker.js"), "utf8"), readFile(path.join(root, "tracker-core.js"), "utf8"), readFile(path.join(root, "trajectory.js"), "utf8"), readFile(path.join(root, "tracking-guard.js"), "utf8"), readFile(path.join(root, "safari-frame-fallback.js"), "utf8"), readFile(path.join(root, "adaptive-source-tracker.js"), "utf8"), readFile(path.join(root, "strict-reacquisition.js"), "utf8")]);
  ...
  for (const phrase of ["class StrictAdaptiveTracker", "confirmationFrames: 3", "flightLockedPoints", "reacquisitionConfirmed"]) if (!strict.includes(phrase)) errors.push(`strict-reacquisition.js is missing ${phrase}.`);
  for (const asset of ["/trajectory.js", "/tracking-guard.js", "/safari-frame-fallback.js", "/adaptive-source-tracker.js", "/strict-reacquisition.js"]) if (!html.includes(asset)) errors.push(`index.html does not load ${asset}.`);
