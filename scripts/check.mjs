...
if (mode === "--typecheck") {
  const [html, app, worker, core, trajectory, guard, frameFallback] = await Promise.all([readFile(path.join(root, "index.html"), "utf8"), readFile(path.join(root, "app.js"), "utf8"), readFile(path.join(root, "tracker.worker.js"), "utf8"), readFile(path.join(root, "tracker-core.js"), "utf8"), readFile(path.join(root, "trajectory.js"), "utf8"), readFile(path.join(root, "tracking-guard.js"), "utf8"), readFile(path.join(root, "safari-frame-fallback.js"), "utf8")]);
  for (const id of ["fileInput", "sourceVideo", "displayCanvas", "analysisCanvas", "scrubber", "trackButton", "resetButton", "exportButton", "shareButton", "statusText"]) { if (!html.includes(`id=\"${id}\"`)) errors.push(`index.html is missing #${id}.`); if (!app.includes(`#${id}`)) errors.push(`app.js does not bind #${id}.`); }
  ...
  for (const phrase of ["requestVideoFrameCallback", "1 / 30", "1400"]) if (!frameFallback.includes(phrase)) errors.push(`safari-frame-fallback.js is missing ${phrase}.`);
  for (const asset of ["/trajectory.js", "/tracking-guard.js", "/safari-frame-fallback.js"]) if (!html.includes(asset)) errors.push(`index.html does not load ${asset}.`);
