(() => {
  ...
  const ui = {
    fileInput: $("#fileInput"), fileButtonText: $("#fileButtonText"), sourceVideo: $("#sourceVideo"),
    displayCanvas: $("#displayCanvas"), analysisCanvas: $("#analysisCanvas"), sourceCropCanvas: $("#sourceCropCanvas"),
    canvasWrap: $("#canvasWrap"), emptyState: $("#emptyState"), scrubber: $("#scrubber"),
    currentTime: $("#currentTime"), duration: $("#duration"), statusText: $("#statusText"),
    confidenceText: $("#confidenceText"), progress: $("#trackProgress"), trackButton: $("#trackButton"),
  ...
  const detectedPoints = () => state.points.filter((point) => point.state === "DETECTED");
  const sourceScaleX = () => ui.sourceVideo.videoWidth / ui.displayCanvas.width;
  const sourceScaleY = () => ui.sourceVideo.videoHeight / ui.displayCanvas.height;
  ...
  function updateButtons() {
    ...
    ui.exportButton.disabled = state.busy || detectedPoints().length < 2;
  ...
  function waitForVideoEvent(eventNames, timeout = 10000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      timer = setTimeout(() => { cleanup(); reject(new Error("The video took too long to decode.")); }, timeout);
    });
  }
  ...
  function configureCanvas() {
    ...
    ui.displayCanvas.width = Math.max(1, Math.round(ui.sourceVideo.videoWidth * scale));
    ui.displayCanvas.height = Math.max(1, Math.round(ui.sourceVideo.videoHeight * scale));
    ui.analysisCanvas.width = ui.displayCanvas.width;
    ui.analysisCanvas.height = ui.displayCanvas.height;
    ui.canvasWrap.style.aspectRatio = `${ui.displayCanvas.width} / ${ui.displayCanvas.height}`;
  }
  ...
  async function drawDecodedFrame() {
    ...
    throw new Error("The video frame could not be displayed.");
  }
  ...
  function captureSourceCrop(centerX, centerY, halfSize) {
    const sourceWidth = ui.sourceVideo.videoWidth;
    const sourceHeight = ui.sourceVideo.videoHeight;
    const size = Math.max(32, Math.round(halfSize * 2));
    const width = Math.min(size, sourceWidth);
    const height = Math.min(size, sourceHeight);
    const originX = Math.round(clamp(centerX - width / 2, 0, sourceWidth - width));
    const originY = Math.round(clamp(centerY - height / 2, 0, sourceHeight - height));
    ui.sourceCropCanvas.width = width;
    ui.sourceCropCanvas.height = height;
  ...
  function captureSourceTemplate() {
  ...
  function recordFrameTime(time) {
    const fps = state.frameRate.add(time);
    ui.sourceVideo.__ballTraceFrameDuration = state.frameRate.frameDuration;
    return fps;
  }

  function reacquireWindowFrames() {
