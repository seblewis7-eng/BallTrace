...
launchSearchRadius: 260,
launchedSearchRadiusMin: 72,
launchedSearchRadiusMax: 340,
detectedThreshold: 0.54,
launchThreshold: 0.5,
reacquireThreshold: 0.58,
maxPredictedGapFrames: 4,
...
const white = hsv.v > 0.56 && hsv.s < 0.38;
const yellow = hsv.v > 0.45 && hsv.s > 0.24 && hsv.h >= 28 && hsv.h <= 82;
...
className: classifyColour(colour.r, colour.g, colour.b),
};
}
...
const hueWeight = templateHsv.s > 0.2 ? 0.32 : 0.07;
const score = 1 - (
rgbDistance * 0.44
+ hueDistance * hueWeight
+ saturationDistance * 0.17
+ valueDistance * (0.39 - hueWeight)
);
...
return clamp(score * (sampleClass === templateClass ? 1 : 0.76), 0, 1);
}
...
b += pb;
count += 1;
if (lum > 0.54 || classifyColour(pr, pg, pb) !== "other") brightCount += 1;
...
function candidateScore({ stats, template, distanceFromPrediction, searchRadius, mode }) {
const colour = colourSimilarity(stats, template);
const sampleClass = classifyColour(stats.r, stats.g, stats.b);
const classMatch = sampleClass === template.className ? 1 : 0;
const motion = clamp(stats.motion * 3.4, 0, 1);
const proximity = Math.exp(-2.4 * (distanceFromPrediction / Math.max(1, searchRadius)) ** 2);
const brightness = clamp(luminance(stats.r, stats.g, stats.b) / Math.max(0.32, template.luminance), 0, 1);
const brightShape = clamp(stats.brightFraction * 2.4, 0, 1);
let score;
if (mode === "launch") {
score = colour * 0.29
+ motion * 0.27
...
return { score, colour, motion, proximity, contrast: stats.contrast, classMatch, brightShape };
}
function findBestCandidate({ frame, previousFrame, width, height, predicted, searchRadius, template, settings, mode = "local", step = settings.gridStep }) {
...
const components = candidateScore({
stats,
template,
...
if (!best || components.score > best.score) best = { x, y, ...components };
}
}
return best;
}
...
const dt = clamp(time - this.previousTime, 1 / 240, 0.35);
const predicted = {
...
? clamp(speed * dt * 1.8 + 58, this.settings.launchedSearchRadiusMin, this.settings.launchedSearchRadiusMax)
...
if (!this.launched && (!candidate || candidate.score < threshold)) {
...
const blend = this.launched ? 0.76 : 0.62;
this.velocity = {
